 ...
 * 变量驱动尺寸（--rx-switch-*）+ 滑块绝对位移；
 * pill 变体完整复刻 Uiverse：太阳渐变滑块 + 选中变月牙（inset shadow）。
 * 默认变体保持 shadcn 原样（无图案）。
 */
function Switch({
  className,
  size = "default",
  ...
  return (
    <SwitchPrimitive.Root
      ...
      className={cn(
        ...
        // pill 变体：Uiverse 复刻（satyamchaudharydev）——胶囊轨道 + 太阳/月亮滑块
        variant === "pill" &&
          "data-[size=default]:h-[2em] data-[size=default]:w-[3.5em] data-[size=sm]:h-[1.6em] data-[size=sm]:w-[2.8em] rounded-[30px] data-unchecked:bg-[#f4f4f5] data-checked:bg-[#303136]",
        className
      )}
      {...props}
    >
      <SwitchPrimitive.Thumb
        ...
        className={cn(
          ...
          // pill 变体：Uiverse 复刻——滑块太阳渐变，选中变月亮（纯色 + 月牙 inset shadow）
          variant === "pill" &&
            "absolute top-1/2 -translate-y-1/2 size-[1.4em] rounded-[20px] bg-[linear-gradient(40deg,#ff0080,#ff8c00_70%)] transition-[left] duration-[400ms] group-data-[size=default]/switch:left-[0.3em] group-data-[size=default]/switch:data-checked:left-[calc(100%-1.7em)] group-data-[size=sm]/switch:left-[0.25em] group-data-[size=sm]/switch:size-[1.2em] group-data-[size=sm]/switch:data-checked:left-[calc(100%-1.45em)] data-checked:bg-none data-checked:bg-[#303136] data-checked:shadow-[inset_-3px_-2px_5px_-2px_#8983f7,inset_-10px_-4px_0_0_#a3dafb]"
        )}
      />
    </SwitchPrimitive.Root>
  )
