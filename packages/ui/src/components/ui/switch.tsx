  ...
  return (
    <SwitchPrimitive.Root
      ...
      className={cn(
        ...
        variant === "pill" &&
          "data-[size=default]:h-[2em] data-[size=default]:w-[3.5em] data-[size=sm]:h-[1.6em] data-[size=sm]:w-[2.8em] rounded-[30px] data-unchecked:bg-[var(--rx-bg-soft)] data-checked:bg-[var(--rx-accent)]",
        className
      )}
      {...props}
    >
      <SwitchPrimitive.Thumb
        ...
        className={cn(
          ...
          // pill 变体：Uiverse 复刻——滑块太阳渐变（accent 系），选中变月亮（accent 系月牙）
          variant === "pill" &&
            "absolute top-1/2 -translate-y-1/2 size-[1.4em] rounded-[20px] bg-[linear-gradient(40deg,var(--rx-accent),var(--rx-accent-strong)_70%)] transition-[left] duration-[var(--rx-dur-slow)] group-data-[size=default]/switch:left-[0.3em] group-data-[size=default]/switch:data-checked:left-[calc(100%-1.7em)] group-data-[size=sm]/switch:left-[0.25em] group-data-[size=sm]/switch:size-[1.2em] group-data-[size=sm]/switch:data-checked:left-[calc(100%-1.45em)] data-checked:bg-none data-checked:bg-[var(--rx-bg-elev)] data-checked:shadow-[inset_-3px_-2px_5px_-2px_var(--rx-accent-soft),inset_-10px_-4px_0_0_var(--rx-accent)]"
        )}
      />
    </SwitchPrimitive.Root>
  )
